...
import {registerOwner, registerStaff, Authorization} from '../../../model/Actions';
...
function AuthForm(props) {
    ...
    const register = [`${globalConsts.server.PATH_STAFF}/api/v1/staff`, 'POST',
        {
            ...
            if (response.errors === null) {
                store.dispatch(registerOwner(response))
                console.log('redirect', props.redirect)
                reactHistory.push(props.redirect);
            } else {
    ...
    const regStaff = [`${globalConsts.server.PATH_STAFF}/api/v1/add_staff?uuid=${props.uuid}&position=${props.position}`,
        'POST',
        {
            'name': formValues.name,
            'email': formValues.email,
            ...
            if (response.errors === null) {
                store.dispatch(registerStaff(response))
                console.log('redirect', props.redirect)
                reactHistory.push(props.redirect);
            } else {
    ...
    const login = [`${globalConsts.server.PATH_STAFF}/api/v1/staff/login`, 'POST',
        {
            ...
            if (response.errors === null) {
                store.dispatch(Authorization(response))
                console.log('redirect', props.redirect)
                reactHistory.push(props.redirect);
            } else {
    ...
    return (
        <Form>
            {console.log('type of form', props.type)}
            <InputField
                text={"Имя"}
                ...
                onValidate={(val) => setFormValidation(
                    formValidation => {
                        return {...formValidation, validName: val}
                    })}
            ...
            <InputField
                ...
                onValidate={(val) => setFormValidation(
                    formValidation => {
                        return {...formValidation, validEmail: val}
                    })}
                onChange={(val) => setFormValues(
                    formValues => {
                        return {...formValues, email: val}
                    })}
            ...
            <InputField
                ...
                onValidate={(val) => setFormValidation(
                    formValidation => {
                        return {...formValidation, validPassword: val}
                    })}
                onChange={(val) => setFormValues(
                    formValues => {
                        return {...formValues, password: val}
                    })}
            ...
            <InputField
                ...
                onValidate={(val) => setFormValidation(
                    formValidation => {
                        return {...formValidation, validRePassword: val}
                    })}
                onChange={(val) => setFormValues(
                    formValues => {
                        return {...formValues, rePassword: val}
                    })}
